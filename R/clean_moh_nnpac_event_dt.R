#' .. content for \description{} (no empty lines) ..
#'
#' .. content for \details{} ..
#'
#' @title
#' @param moh_nnpac_event_raw_dt
#' @param event_end_type_lookup_dt
#' @return
#' @author Matthew Moore
#' @export
clean_moh_nnpac_event_dt <- function(moh_nnpac_event_raw_dt,
                                     event_end_type_lookup_dt) {

  moh_nnpac_event_dt = merge(moh_nnpac_event_raw_dt,
                             event_end_type_lookup_dt,
                             by = 'END_TYPE',
                             all.x = TRUE)
  
  moh_nnpac_event_dt[, `:=`(
    # event_date = as.Date(event_date, format = "%Y-%m-%d"),
    # event_time = as.POSIXct(event_time, format = "%H:%M:%S"),
    EVENT_START_DATETIME  = as.POSIXct(EVENT_START_DATETIME, format = "%d%b%Y:%H:%M:%S"),
    EVENT_END_DATETIME  = as.POSIXct(EVENT_END_DATETIME, format = "%d%b%Y:%H:%M:%S"),
    LAST_UPDATED_DATE = lubridate::dmy(LAST_UPDATED_DATE)
  )]
  
  setcolorder(moh_nnpac_event_dt,
              c(
                'EVENT_ID',
                'PMS_UNIQUE_IDENTIFIER',
                'supplied_nhi',
                'PRIM_HCU',
                'EVENT_START_DATETIME',
                'EVENT_END_DATETIME'
              ))
  
  moh_nnpac_event_dt = unique(moh_nnpac_event_dt)
  
  return(moh_nnpac_event_dt)

}
